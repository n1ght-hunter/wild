...
fn subprocess_result() -> Result<i32> {
    ...
    match unsafe { fork(&mut hprocess, &mut hthread) } {
        STATUS_PROCESS_CLONED => {
...

            {
                ...
                let _outputs = linker.run()?;
                println!("Linker has finished running, notifying parent...");
        ...
        _ => {
            eprintln!("Fork failure in the parent - Fallback to running linker in this process");
...
fn inform_parent_done(write_end: HANDLE) {
    ...
    unsafe {
        ...
        println!("Parent informed that the linker has finished.");
        CloseHandle(write_end);
        println!("Closing write end of the pipe.");
        // FreeConsole();
    }
}
...
fn wait_for_child_done(read_end: HANDLE, hprocess: HANDLE, hthread: HANDLE) -> i32 {
    ...
    match unsafe {
        ...
        TRUE => {
            println!(
                "Child sent a byte, which indicates that it succeeded and is now shutting down in the background."
            );
        ...
        _ => {
            eprintln!("Child closed pipe without sending a byte - getting process exit status");
